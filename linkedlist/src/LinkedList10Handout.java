...
{
   // calls find 
	LinearNode newnode = new LinearNode(addData,null);
	current = find(target);
...
LinearNode node = new LinearNode(target);
  ...
  /*******Before insertAfter is called current must be pointing to A  node on the list
    ...
    { 
    	  LinearNode newnode = new LinearNode(newData, null);
   	   if(isEmpty())
   	   {// is the list is empty
   	      head =  newnode; 
...
// SEVERAL SITUATIONS MUST BE HANDLED AS LISTED BELOW
   ...
   public LinkedList10Handout<T> printbackwards() throws EmptyCollectionException //*******************************
   {
	   LinkedList10Handout<T> list2 = new LinkedList10Handout<T>();
...
class LinearNode<T>
 {
    ...
    public LinearNode (T elem)
    {
       ...
       public LinearNode(T  initialData, LinearNode next)
       {
           element = initialData;   // data for the node
